...
import { createColdStartAgentTemplates } from "../lib/agent-registry";
...
describe("coordinationOverheadTokens", () => {
  it("matches (selected_count + 1) * request_tokens * 16", () => {
    const profiles = createColdStartAgentTemplates().slice(0, 5);
...
describe("selectedAgentHistoricalTokens", () => {
  it("uses task-specific historical tokens where available and falls back to general history otherwise", () => {
    const profiles = createColdStartAgentTemplates();
    const top = profiles.filter((agent) =>
      ["agent-evidence", "agent-finance", "agent-legal", "agent-crypto", "agent-technical"].includes(
        agent.agentId
      )
    );
    // README's per-agent historical tokens: 9415 + 7727 + 7816 + 7640 + 7391 = 39_989.
    expect(selectedAgentHistoricalTokens("crypto_market_decision", top)).toBe(39_989);
  });
...
describe("buildGovernancePlan", () => {
  it("reproduces the README's worked-example budget for the Coinbase prompt", () => {
    const candidates = createColdStartAgentTemplates();
    ...
    expect(plan.totalTokenBudget).toBe(69_000);
    ...
    expect(plan.budgetEstimate.selectedAgentHistoricalTokens).toBe(39_989);
    expect(plan.budgetEstimate.reserveTokens).toBe(9660);
