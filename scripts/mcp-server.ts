...
(server as { registerTool: (...args: unknown[]) => unknown }).registerTool = (name: unknown, config: unknown, handler: unknown) => {
  ...
  return (__originalRegisterTool as unknown as (n: unknown, c: unknown, h: unknown) => unknown)(name, config, safeHandler);
};
